<template>
    <div >
        ...
        <div id="card" class="card columns is-multiline" style="height:85vh; margin:30px; padding: 20px">
            <div class="column is-full">
                <div style="display: inline-block">
                    <img :src="'../img/' + firstDice" height="100" style="margin-right:35px" width="100" alt="Placeholder image">             
                </div>
                <div style="display: inline-block" >
                    <img :src="'../img/' + secondDice " height="100"  width="100" alt="Placeholder image">
                </div>            
                <div v-if="report" id=report>
                    <p v-if="room.dices[room.stage-1]" >its {{room.dices[room.stage-1].firstDice + room.dices[room.stage-1].secondDice}}!</p>
                </div>
            </div>
            <div class="column is-full columns is-multiline" >     
                ...
                <div class="column is-full">
                    <b-field v-if="playing" label="Guess a number between 2 - 12!">
                        <b-tooltip label="guess between 2 - 12" >
                            <b-numberinput  v-model = "selected" min="2" max="12">
                            </b-numberinput>
                        </b-tooltip>
                    </b-field>  
                ...
                <div class="column is-full" v-if="over" >
                    ...
                    <div class="card scores" >
                        <h1 style="margin-bottom: 10px"  v-for="player in room.players" :key="player.id">{{player.username}} score: {{player.score}}</h1>    
                    </div>
                </div>
            </div>
        </div> 
...
export default {
    ...
    methods:{
        rollDice(one,two){
            ...
            setTimeout(()=>{
                if(this.submitted){
                    let result = Math.abs(this.fixSelected-(this.room.dices[this.room.stage-1].firstDice + this.room.dices[this.room.stage-1].secondDice))
    ...
    watch:{
        ...
        stage(newVal, oldVal){
            if(oldVal < 3){
                this.over = false
                this.rollDice(this.room.dices[newVal-1].firstDice,this.room.dices[newVal-1].secondDice)
                setTimeout(()=>{
